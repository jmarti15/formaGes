...
//System.out.println( "paramMap:"+request.getParameterMap() );		// log
	...
	if( sMtd.equals("GET")){						// db select
		...
		if(sCodi==null) {								// per la taula
			...
			FROM Alumnes I JOIN Poblacions P ON I.CodPobl = P.Codi
			*/
			...
			sSql += "FROM Alumnes I JOIN Poblacions P ON I.CodPobl = P.Codi";
			jsonString = sSelect2Json( sSql, false );
		} else {												// pel form
			/*
			SELECT Codi, Nom, Cognoms, Adre, CodPobl, Telef1, Telef2, Email, DAlta, Coment, CodMetodeCapt, Curriculum, Formacio, CodCentre 
			FROM Alumnes WHERE Codi = 1 
			*/
			sSql = "SELECT Codi, Nom, Cognoms, Adre, CodPobl, Telef1, Telef2, Email, DAlta, Coment, CodMetodeCapt, Curriculum, Formacio, CodCentre ";
			sSql += "FROM Alumnes WHERE Codi="+sCodi;
...
// {"DAlta":"2015-05-02","Nom":"Jordi","Cognoms":"MC","Telef1":"605105350","Email":"dakar@tinet.cat","Adre":"Sales,+36","CodPobl":"38370"}
		/*
		INSERT INTO Alumnes (Nom, ..) VALUES ("Hipo N4", ..)
		*/
		String sSql = sInsertQuery( "Alumnes", sData );
...
//	{"Codi":"1","Nom":"Joan","Cognoms":"Segarra+Alzina","Adre":"Maragall,+121","CodPobl":"12222","Telef1":"653105222","Telef2":null,"Email":"jsa@gmail.com","DAlta":"2015-08-18T00:00:00.000Z","CodOrganitzador":"0","PendentObrir":"0","Coment":""}
		/*
		UPDATE Alumnes SET Nom="LPF N3", ... WHERE Codi=3
		*/
		String sSql = sUpdateQuery( "Alumnes", sData );
		int iRes = iExecQuery( sSql );
		JSONObject json = new JSONObject();
		json.put("res", iRes);
		jsonString = json.toJSONString();
		...
		String sCodi = request.getParameter("codi");
		/*
		DELETE FROM Alumnes WHERE Codi=6
		*/
		String sSql = "DELETE FROM Alumnes WHERE Codi="+sCodi;
		int iRes = iExecQuery( sSql );
		JSONObject json = new JSONObject();
		json.put("res", iRes);
		jsonString = json.toJSONString();
