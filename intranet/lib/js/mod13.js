...
var mod13 = angular.module('mod13', ['scrollable-table', 'ui.bootstrap']);

mod13.service('func', function() {
...
mod13.directive('datepickerPopup', function (dateFilter, datepickerPopupConfig) {
	return {
		restrict: 'A',
		priority: 1,
		require: 'ngModel',
...
mod13.controller('opcCtrl', function($scope, $http, $modal, func) {
	...
	$scope.getData = function() {
		$http.get("api/alumnes.jsp").success( function(data) {
		    $scope.data = data;
	...
	$scope.detall = function(codi) {
		if(codi) {
			$http.get("api/alumnes.jsp?codi="+codi).success( function(data) {
...
mod13.controller('detallCtrl', function($scope, $http, $timeout, $modalInstance, dateFilter, func, dataDet) {
// dataDet:	mod/del			{"Codi":"1","Nom":"Jordi","Cognoms":"Martí Cisteré","Adre":"Sales i Ferré, 36 SOT 1","CodPobl":"35171",
//													"Telef1":"653 105 350","Telef2":"tel2","Email":"jordi@tinet.cat","DAlta":"2015-03-08","Coment":"assaa\nsss",
//													"CodMetodeCapt":"1","Curriculum":"","Formacio":"","CodCentre":"0"}
//						add					null
...
//$scope.poblacions = [{"Codi":"34987","Pobl":"MASO, LA","Prov":"Tarragona"},{"Codi":"34997","Pobl":"MILA, EL","Prov":"Tarragona"}]
			...
			var idxMC = document.getElementById('metCap').selectedIndex;
			$scope.data.CodMetodeCapt = idxMC===0 ? null : $scope.metodesCap[idxMC-1].Codi;
		
		} else if( opc=='centre' ){
			// al onKeyUp encara no s'ha actualitzat el valor de data.CodCentre
			var idxC = document.getElementById('centre').selectedIndex;
...
*/
		...
		// A la taula, el camp és tipus int. Si al desplegable hem seleccionat "" tindrem null, ho canviem per 0 per desar-ho a la taula
		if( $scope.data.CodMetodeCapt === null ) $scope.data.CodMetodeCapt = 0;
		if( $scope.data.CodCentre === null ) $scope.data.CodCentre = 0;
	...
	$scope.add = function() {
		...
		$http.post("api/alumnes.jsp", null, {"params":{"data": $scope.data}} )
	...
	$scope.mod = function() {
		...
		$http.put("api/alumnes.jsp", null, {"params":{"data": $scope.data}} )
		.success(function(data) {
			if(data.res > 0) {
				$modalInstance.close();
			} else {
...
//		if( !$scope.validaDel() ) return;
		...
		if( !$scope.delPremut ) {
			...
			$http.delete("api/alumnes.jsp", {"params":{"codi": $scope.codi}})
			.success(function(data) {
				if(data.res > 0) {
					$modalInstance.close();
				} else {
